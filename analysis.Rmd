...
```{r}
df$log_salary_millions <- log(df$avg_salary_millions + 1)
hist(df$log_salary_millions, main = "Distribution of Log(Average Salary)", 
     xlab = "Log(Average Salary)", col = "blue")
```
...
```{r}
cv_model <- train(
  avg_salary_millions ~ MP + PTS + TRB + AST + awards_1 + awards_2plus + Age_22_26 + Age_27_31 +   Age_32_34 + Age_35_plus + Pos_PF + Pos_PG + Pos_SF + Pos_SG, data = df,
  method = "lm",
