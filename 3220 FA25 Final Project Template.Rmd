...
## Data Summary
### Data Sources
Our dataset includes all NBA players who appeared during the 2024–2025 season, combining salary, performance, demographic, and award information. Player statistics and awards were sourced from Basketball Reference, while Statista provided team franchise values to add economic context. After standardizing player names, we merged all sources so each player occupied one row. Awards were simplified into a three-level categorical variable, and age was grouped into brackets. All players who logged minutes were retained to preserve the natural distribution of roles and contracts. The resulting dataset is comprehensive, well-structured, and suitable for analyzing how performance and recognition relate to NBA salary variation.
...
df <- df[df$MP > 20, ]
df$sqrt_sal <- sqrt(df$avg_salary_millions + 1)

p1 <- ggplot(df, aes(x = sqrt_sal)) +
  geom_histogram(binwidth = 0.25, fill = "blue", color = "black") +
  labs(title = "Distribution of sqrt-transformed Salaries",
       x = "sqrt(Salary + 1)", y = "Count") +
  theme_minimal()

p2_salary_vs_pts <- ggplot(df, aes(x = PTS, y = sqrt_sal)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "sqrt(Salary) vs Points Per Game",
       x = "Points per game", y = "sqrt(Salary + 1)") +
  theme_minimal()

p3_salary_vs_stocks <- ggplot(df, aes(x = stocks, y = sqrt_sal)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "sqrt(Salary) vs Defensive Stocks (Steals + Blocks)",
       x = "Stocks (steals + blocks)", 
       y = "sqrt(Salary + 1)") +
  theme_minimal()

p4_salary_by_awards <- ggplot(df, aes(x = Awards, y = sqrt_sal)) +
  geom_boxplot(fill = "grey85") +
  labs(title = "sqrt(Salary) by Awards Category",
       x = "Awards Category", y = "sqrt(Salary + 1)") +
  theme_minimal()

p6_salary_by_age <- ggplot(df, aes(x = Age, y = sqrt_sal)) +
  geom_boxplot(fill = "lightgray") +
  labs(title = "sqrt(Salary) by Age Group",
       x = "Age Group", y = "sqrt(Salary + 1)") +
  theme_minimal()
...
### EDA Summary
NBA salaries are heavily right-skewed: most players earn under \$10 million, while a small superstar group reaches \$40–\$60 million. The mean exceeds the median, but the variable still has adequate spread for linear modeling.

Scoring shows the strongest salary relationship—20+ PPG players often earn \$35–\$45 million, compared to under \$5 million for low scorers. Defensive stocks have a more modest positive association, indicating that defense matters but is less financially influential than scoring.

Awards correspond to major salary jumps: players with no awards earn about \$8 million, while those with one or more awards earn \$50–\$55 million, reflecting the added value of recognized excellence.

Salary patterns across age groups align with typical NBA career arcs, peaking at ages 27–31 and tapering for younger and older players.

Initial checks show little multicollinearity among PTS, stocks, awards, and age, which supports their suitability for modeling.

Overall, the EDA highlights five patterns: salaries are right-skewed, scoring is the strongest predictor, defense shows moderate influence, awards correlate with large pay increases, and earnings peak mid-career. These results provide a clear rationale for our chosen predictors moving into regression modeling.
```{r, echo=FALSE, warning=FALSE,results=FALSE}
